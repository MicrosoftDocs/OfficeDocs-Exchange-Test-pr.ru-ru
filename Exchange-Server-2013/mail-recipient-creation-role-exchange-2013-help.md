---
title: 'Роль создания получателя электронной почты: Exchange 2013 Help'
TOCTitle: Роль создания получателя электронной почты
ms:assetid: 81081845-9ebf-4888-8f9f-fe7cf2704486
ms:mtpsurl: https://technet.microsoft.com/ru-ru/library/Dd876915(v=EXCHG.150)
ms:contentKeyID: 50488520
ms.date: 05/22/2018
...
Роль управления `Mail Recipient Creation` используется администраторами для создания почтовых ящиков, пользователей почты, почтовых контактов, групп рассылки и динамических групп рассылки в организации. Для создания и управления получателями можно совместно использовать эту роль управления и роль `Mail Recipients`. Дополнительные сведения см. в разделе [Роль получателей почты](mail-recipients-role-exchange-2013-help.md).

С помощью этой роли нельзя включить поддержку почты для общих папок. Чтобы включить поддержку почты для общих папок, следует использовать роль `Mail Enabled Public Folders`. Дополнительные сведения см. в разделе [Роль общедоступных папок с включенной поддержкой почты](mail-enabled-public-folders-role-exchange-2013-help.md).

Если в организации используется модель разделения разрешений "Управление доступом на основе ролей (RBAC)", в которой создание получателей и управление ими осуществляется разными группами пользователей, назначьте роль `Mail Recipient Creation` группе ролей управления, которая создает получателей, а роль `Mail Recipients` — группе ролей, которая управляет получателями.
...
Для получения дополнительных сведений об управлении доступом на основе ролей и разрешениях разделения Active Directory см. [Общие сведения о разделенных разрешениях](understanding-split-permissions-exchange-2013-help.md).
...
Помимо областей получателей с помощью областей конфигурации базы данных в область также включен командлет **New-Mailbox**, содержащийся в роли. Области конфигурации базы данных управляют возможностью создания нового почтового ящика в базе данных посредством командлета. База данных, в которой требуется создать почтовый ящик, должна находиться в области базы данных. Это условие применимо при указании базы данных с помощью параметра *Database* для командлета **New-Mailbox** или при использовании автоматического распределения почтовых ящиков для выбора базы данных. Дополнительные сведения см. в разделе [Общие сведения об областях ролей управления](understanding-management-role-scopes-exchange-2013-help.md).
