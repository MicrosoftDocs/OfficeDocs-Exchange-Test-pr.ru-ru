...
> [!NOTE]  
> Поиск слов и текстовых шаблонов в теме или других полях заголовка сообщения выполняется <em>после</em> расшифровки сообщения, к которому применялся метод кодирования для передачи сообщений MIME. Этот метод используется для передачи двоичных сообщений между SMTP-серверами с преобразованием их в текст ASCII. Вы не можете применять условия или исключения для поиска необработанных закодированных значений (обычно в формате Base64) в теме или других полях заголовка сообщения.
...
> [!NOTE]  
> Если Outlook или Outlook Web App настроен для пересылки сообщения, свойство <strong>ForwardingSmtpAddress</strong> добавляется к сообщению. Тип сообщения не будет изменено на <code>AutoForward</code>.
...
> [!NOTE]  
> Если свойство — это строка, конечные пробелы не допускаются.
...
> [!NOTE]  
> Чтобы определить, где находятся почтовые контакты (в организации или вне ее), адрес отправителя сравнивают со списком обслуживаемых доменов организации.
