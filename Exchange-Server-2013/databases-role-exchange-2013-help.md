---
title: 'Роль баз данных: Exchange 2013 Help'
TOCTitle: Роль баз данных
ms:assetid: 317d9945-668b-448e-963f-f7127b9b3d79
ms:mtpsurl: https://technet.microsoft.com/ru-ru/library/Dd876868(v=EXCHG.150)
ms:contentKeyID: 50487766
ms.date: 05/22/2018
...
Роль управления `Databases` позволяет администраторам создавать, управлять, подключать и отключать базы данных почтовых ящиков на определенных серверах.
...
Для командлета **Move-DatabasePath**, включенного в эту роль, необходимо, чтобы база данных, которую требуется настроить, находилась в области баз данных, а также на сервере, входящем в область серверов.

Кроме того, для выполнения командлета **Remove-MailboxDatabase**, который также включен в эту роль, требуется, чтобы удаляемая база данных была в области действия базы или база данных должна храниться на сервере в области сервера. Это значит, что вы управляете тем, кто может удалять базы данных почтовых ящиков с помощью областей базы данных или сервера.

Дополнительные сведения см. в разделе [Общие сведения об областях ролей управления](understanding-management-role-scopes-exchange-2013-help.md).
