...
> [!IMPORTANT]  
> В среде, в которую интегрированы единая система обмена сообщениями и сервер МайкрософтOffice Communications Server 2007 R2 или Microsoft Lync Server, отправка и получение факсов по протоколам T.38 или G.711 не поддерживаются.
...
Чтобы включить возможность получения факсов для пользователей с поддержкой единой системы обмена сообщениями, необходимо выполнить описанные ниже действия.
  ...
  - Дополнительные сведения см. в разделе [Установка сервера факсов URI, чтобы разрешить отправка и прием факсов партнера](set-the-partner-fax-server-uri-to-allow-faxing-exchange-2013-help.md).
    ...
    > [!WARNING]  
    > Хотя вы можете включить несколько записей в формате для <em>FaxServerURI</em>, разделив их точками с запятыми, будет использована только одна запись. Этот параметр разрешает использовать только одну запись, и добавление нескольких записей не позволит распределить нагрузку для факс-запросов.
